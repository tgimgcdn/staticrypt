...
const staticrypt = (function() {
    ...
    const init = function(config) {
        ...
        // 解密并替换标记的内容
        async function decryptAndReplaceHtml(hashedPassword, sectionId = null) {
            ...
            try {
                ...
                // 如果指定了 sectionId，只解密该部分
                if (sectionId) {
                    const section = encryptedSections.find(s => s.id === sectionId);
                    if (section) {
        ...
        // 处理密码解密
        async function handleDecryptionOfPage(password, isRememberChecked, sectionId = null) {
            const hashedPassword = await cryptoEngine.hashPassword(password, config.staticryptSaltUniqueVariableName);
            const isSuccessful = await decryptAndReplaceHtml(hashedPassword, sectionId);
